...
export default class SwipeableItemRow extends Component<IProps, IState> {
  ...
  options(props: {onDelete: () => void}) {
    ...
    const limit = width - SwipeableItemRow.optionsWidth;
      ...
      <Div row alignItems="center" pt={12}>
        <Button
          ...
          <Div row justifyContent="space-between" w="100%" alignItems="center">
            {this.text(this.props.title, 6)}
            {this.textRub(this.props.price, 2)}
          </Div>
        </Button>
        <Div
          row
        ...
        >
          <SwipeableItem
            ...
            <Div row opacity={this.state.opacity}>
              {this.text(this.props.quantity, 1, true)}
              {this.textRub(this.props.price * this.props.quantity, 2)}
            </Div>
          </SwipeableItem>
        </Div>
      </Div>
